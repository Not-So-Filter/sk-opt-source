...
dma68kToVDP macro source,dest,length,type
 ...
 ; tells the VDP to copy a region of 68k memory to VRAM or CRAM or VSRAM
dma68kToVDPsrcdefined macro source,dest,length,type
	move.l	#(($9400|((((length)>>1)&$FF00)>>8))<<16)|($9300|(((length)>>1)&$FF)),(a5)
	move.l	#(($9600|((((source)>>1)&$FF00)>>8))<<16)|($9500|(((source)>>1)&$FF)),(a5)
	move.w	#$9700|(((((source)>>1)&$FF0000)>>16)&$7F),(a5)
	move.w	#(vdpComm(dest,type,DMA)>>16)&$FFFF,(a5)
...
clearRAM macro addr,length
    ...
    if (length - ((addr)&1)<$200)
	moveq	#bytesToLcnt(length - ((addr)&1)),d1
    else
...
.loop:	beq.s	.loop ; loop until it says it's stopped
    endm
...
pcmLoopCounter function sampleRate, pcmLoopCounterBase(sampleRate,92) ; 92 is the number of cycles zPlaySEGAPCM takes to deliver one sample.
dpcmLoopCounter function sampleRate, pcmLoopCounterBase(sampleRate,257/2) ; 257 is the number of cycles zPlayDigitalAudio takes to deliver one sample.
